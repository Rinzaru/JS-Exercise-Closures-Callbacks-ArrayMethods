...
function processDuplicateFree(list, callBack) {
  ...
  const deDup = list.filter((a, b) => list.indexOf(a) === b);
...
function lowerCaseStrings(strings) {
  /* code here */
  const newArr = [];
  strings.forEach((element) => newArr.push(element.toLowerCase()));
  return newArr;
}
...
function stringSmash(strings) {
  ...
  return strings.reduce(reducing);
}
...
const customRunners = require("./data/runners");

function getFullNames(runners) {
  /* CODE HERE */

  const firstLast = runners.map((x) => `${x.last_name}, ${x.first_name}`);
  return firstLast;
}

// console.log(getFullNames(testRunners));
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  ...
  let shirtSize = runners.filter((word) => word.shirt_size === tShirtSize);
...
function tallyUpDonations(runners) {
  /* CODE HERE */
  let newArr = [];
  let donations = newArr.push(runners.donations);
  return newArr;
}

console.log(tallyUpDonations(customRunners));
